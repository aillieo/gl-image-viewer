...
// 三个点光源 红绿蓝
int multipleLightsRGB(GLFWwindow* window)
{
    // 开启深度检测
    glEnable(GL_DEPTH_TEST);
    ...
    // SHADER
    Shader* shaderLight = new Shader("MultipleLights/shader3.vsh","MultipleLights/shader3.fsh");
    Shader* shader = new Shader("MultipleLights/shader4.vsh","MultipleLights/shader4.fsh");
    ...
    while(!glfwWindowShouldClose(window) && !SceneManager::willChangeScene)
    {
        ...
        for (int i = 0; i < sizeof(cubePositions)/sizeof(cubePositions[0]); i++)
        {
            ...
            for(int j = 0; j < sizeof(pointLightPositions)/sizeof(pointLightPositions[0]); j++)
            {
                ...
                glm::vec3 lightColor = pointLightColors[j];
