...
{
    return Shader::setMat4( name, glm::value_ptr(value));
}
...
void Shader::init(const GLchar* vertexPath, const GLchar* geometryPath, const GLchar* fragmentPath)
{
    ...
    try
    {
        std::stringstream vShPath,gShPath,fShPath;
        vShPath<<shaderPath<<vertexPath;
        gShPath<<shaderPath<<geometryPath;
        fShPath<<shaderPath<<fragmentPath;
        
        // open files
        vShaderFile.open(vShPath.str().c_str());
        gShaderFile.open(gShPath.str().c_str());
        fShaderFile.open(fShPath.str().c_str());
        ...
        // convert stream into string
        vertexCode   = vShaderStream.str();
        geometryCode = vShaderStream.str();
        fragmentCode = fShaderStream.str();
    }
    catch (std::ifstream::failure e)
    {
